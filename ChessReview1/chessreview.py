...
def analyze_pgn_with_stockfish(pgn_file: str, engine_path: str, book_csv: str):
    ...
    with chess.engine.SimpleEngine.popen_uci(engine_path) as engine:
        ...
        # Inside the analyze_pgn_with_stockfish function
        for move_number, node in enumerate(game.mainline(), start=1):
            ...
            # Print move analysis in the new format
            print(f"Move {move_number} ({game.headers['White' if board.turn == chess.BLACK else 'Black']}): {move}")
            print(f"  Evaluation: {post_eval/100:.2f} centipawns")
            print(f"  Evaluation Loss: {eval_loss/100:.2f} centipawns")
            print(f"  Classification: {classification.value.lower()}\n")
    ...
    for phase in GamePhase:
        ...
        if moves:
            ...
            print(f"{phase.value.capitalize()} ({len(moves)} moves): {rating.value}")

    print("\nPlayer Summaries:")
    for color in ["white", "black"]:
        player = game.headers["White" if color == "white" else "Black"]
        print(f"\n{player}'s Performance:")
        total = sum(len(m) for m in classifications[color].values())
        counts = {c: 0 for c in Classification}
        
        for phase in GamePhase:
            phase_moves = classifications[color][phase]
            if phase_moves:
                rating = get_phase_rating(phase_moves)
                print(f"  {phase.value.capitalize()}: {rating.value} ({len(phase_moves)} moves)")
                for m in phase_moves:
                    counts[m] += 1
        
        print("\n  Total Classifications:")
        for c in Classification:
            print(f"  {c.value}: {counts[c]}")

# Example usage
analyze_pgn_with_stockfish(
    "game2.pgn",
