...
async def analyze_pgn(json_file: UploadFile = File(...), engine_path: str = "stockfish-windows-x86-64.exe", book_csv: str = "openings_master.csv"):
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".json") as tmp_file:
            tmp_file.write(await json_file.read())
        ...
        # Check if the JSON file contains a list of PGNs
        if not isinstance(pgn_data, list):
            raise HTTPException(status_code=400, detail="JSON file should contain a list of PGNs.")

        # Analyze each PGN in the list
        analysis_results = []
        for pgn_content in pgn_data:
            # Save the PGN content to a temporary file
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pgn") as pgn_tmp_file:
                pgn_tmp_file.write(pgn_content.encode('utf-8'))
                pgn_tmp_file_path = pgn_tmp_file.name

            # Analyze the PGN file
            analysis_result = analyze_pgn_with_stockfish(pgn_tmp_file_path, engine_path, book_csv)
            analysis_results.append(analysis_result)

            # Clean up the temporary PGN file
            os.remove(pgn_tmp_file_path)
